...
/// Response for get category endpoint
#[derive(Debug, Clone)]
...
/// Use case for getting a single issue category
...
impl<C, P, U, M> GetCategoryUseCase<C, P, U, M>
where
    C: IssueCategoryRepository,
    P: ProjectRepository,
    U: UserRepository,
    M: MemberRepository,
{
    ...
    ///
    /// Returns a single issue category by ID.
    ...
    pub async fn execute(
        ...
        current_user_id: i32,
        is_admin: bool,
    ) -> Result<GetCategoryResponse, ApplicationError> {
        // 1. Get category
        let category = self
            .category_repo
        ...
        if !is_admin {
            ...
            if !is_member && !project.is_public {
                return Err(ApplicationError::Forbidden(
                    "You don't have permission to view this category".into(),
                ));
            }
        }
...
mod tests {
    ...
    #[tokio::test]
    async fn test_get_category_as_admin() {
        ...
        let project_repo = Arc::new(MockProjectRepository {
            projects: vec![create_test_project(1, true)],
        });
        ...
        let usecase = GetCategoryUseCase::new(category_repo, project_repo, user_repo, member_repo);

        let result = usecase.execute(1, 1, true).await.unwrap();
    ...
    #[tokio::test]
    async fn test_get_category_with_assignee() {
        let category = create_test_category(1, 1, "Backend", Some(2));
        let category_repo = Arc::new(MockCategoryRepository::with_category(category));
        let project_repo = Arc::new(MockProjectRepository {
            projects: vec![create_test_project(1, true)],
        });
        ...
        let assignee = result.issue_category.assigned_to.unwrap();
        assert_eq!(assignee.id, 2);
        assert_eq!(assignee.name, "First2 Last2");
    }

    #[tokio::test]
    async fn test_get_category_as_member_of_public_project() {
        let category = create_test_category(1, 1, "Testing", None);
        let category_repo = Arc::new(MockCategoryRepository::with_category(category));
        let project_repo = Arc::new(MockProjectRepository {
            projects: vec![create_test_project(1, true)],
        });
        ...
        let member_repo = Arc::new(MockMemberRepository::with_member(1, 2, vec![3]));

        let usecase = GetCategoryUseCase::new(category_repo, project_repo, user_repo, member_repo);

        let result = usecase.execute(1, 2, false).await.unwrap();
        assert_eq!(result.issue_category.id, 1);
        assert_eq!(result.issue_category.name, "Testing");
    }

    #[tokio::test]
    async fn test_get_category_public_project_non_member() {
        let category = create_test_category(1, 1, "Public Category", None);
        let category_repo = Arc::new(MockCategoryRepository::with_category(category));
        let project_repo = Arc::new(MockProjectRepository {
            projects: vec![create_test_project(1, true)], // Public project
        });
        let user_repo = Arc::new(MockUserRepository {
            users: vec![create_test_user(2, false)],
        });
        let member_repo = Arc::new(MockMemberRepository::new()); // Not a member

        let usecase = GetCategoryUseCase::new(category_repo, project_repo, user_repo, member_repo);

        // Non-member should be able to view public project categories
        let result = usecase.execute(1, 2, false).await.unwrap();
        assert_eq!(result.issue_category.id, 1);
        assert_eq!(result.issue_category.name, "Public Category");
    }

    #[tokio::test]
    async fn test_get_category_private_project_permission_denied() {
        let category = create_test_category(1, 1, "Private Category", None);
        let category_repo = Arc::new(MockCategoryRepository::with_category(category));
        let project_repo = Arc::new(MockProjectRepository {
            projects: vec![create_test_project(1, false)], // Private project
        });
        let user_repo = Arc::new(MockUserRepository {
            users: vec![create_test_user(2, false)],
        });
        ...
        let result = usecase.execute(1, 2, false).await;
    ...
    #[tokio::test]
    async fn test_get_category_private_project_as_member() {
        let category = create_test_category(1, 1, "Private Category", None);
        let category_repo = Arc::new(MockCategoryRepository::with_category(category));
        let project_repo = Arc::new(MockProjectRepository {
            projects: vec![create_test_project(1, false)], // Private project
        ...
        let result = usecase.execute(1, 2, false).await.unwrap();
        assert_eq!(result.issue_category.id, 1);
        assert_eq!(result.issue_category.name, "Private Category");
    }
    ...
    async fn test_get_category_not_found() {
        ...
        let result = usecase.execute(999, 1, true).await;
        assert!(result.is_err());
        assert!(matches!(result.unwrap_err(), ApplicationError::NotFound(_)));
    }

    #[tokio::test]
    async fn test_get_category_project_not_found() {
        let category = create_test_category(1, 999, "Orphan Category", None);
        let category_repo = Arc::new(MockCategoryRepository::with_category(category));
        let project_repo = Arc::new(MockProjectRepository {
        ...
        let result = usecase.execute(1, 1, true).await;
        assert!(result.is_err());
        assert!(matches!(result.unwrap_err(), ApplicationError::NotFound(_)));
    }
}
